  ...
  return <View style={ui.page}>
    <ScrollView contentContainerStyle={ui.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[ui.row, { flexGrow: 1 }]}>
        {['الكل', 'قهوة مختصة', 'جلسات خارجية'].map(label => <Pill key={label} label={label} active={category === label} onPress={() => setCategory(label)} />)}
      </ScrollView>
      <View style={ui.between}><Text style={ui.heading}>على ذوقك</Text><Text style={s.count}>{cafes.length} مقاهي</Text></View>
    ...
    <Sheet visible={citiesOpen} onClose={() => setCitiesOpen(false)} title="وين قهوتك اليوم؟">
      {['الرياض', 'جدة', 'الخبر'].map(label => <Button key={label} label={label} secondary={city !== label} onPress={() => { setCity(label); setCitiesOpen(false); }} />)}
