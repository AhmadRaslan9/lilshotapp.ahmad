...
export default function CafeDetailScreen({ cafe, onClose }) {
  ...
  return <SafeAreaView style={ui.page}><ScrollView contentContainerStyle={{ paddingBottom: 36 }} showsVerticalScrollIndicator={false}>
    ...
    <View style={{ paddingHorizontal: 22, gap: 22 }}>{cafe.isLive ? <DemoNote>حساب مقهى حقيقي · المنيو يتحدّث مباشرة</DemoNote> : <DemoNote>حساب مقهى تجريبي · بيانات وأسعار توضيحية</DemoNote>}
      ...
      {cafe.isLive ? <Button label="المتابعة قريباً" icon="people-outline" secondary disabled /> : <><Button label={pending ? 'إلغاء طلب المتابعة' : 'طلب متابعة'} icon={pending ? 'time-outline' : 'add'} onPress={() => toggleRequest(cafe.id)} secondary={pending} /><Text style={[ui.subtitle,{ textAlign: 'center' }]}>{pending ? 'طلب تجريبي معلّق؛ لم يُرسل إلى متجر حقيقي.' : 'طلبات المتابعة تحتاج قبول صاحب الحساب.'}</Text></>}
