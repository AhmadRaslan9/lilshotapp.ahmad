  ...
  return <View style={ui.page}>
    ...
    {loadError ? <View style={s.center}><Text style={s.error}>{loadError}</Text></View> : loading ? <View style={s.center}><ActivityIndicator color={c.accent} size="large" /></View> :
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {!items.length ? <View style={s.empty}><View style={s.emptyIcon}><Ionicons name="restaurant-outline" size={34} color={c.accent} /></View><Text style={s.emptyTitle}>منيوك ينتظر أول صنف</Text><Text style={s.emptyText}>{canManage ? 'أضف القهوة أو الحلو المميز عندك، وسيبقى محفوظاً في Firestore.' : 'بعد تفعيل الاشتراك ستتمكن من إضافة الأصناف.'}</Text></View> : grouped.map((group) => <View key={group.category} style={s.group}>
          ...
          {group.items.map((item) => <TouchableOpacity key={item.id} disabled={!canManage} onPress={() => openEdit(item)} style={[s.item, !item.available && s.unavailable]}>
            ...
            <View style={s.itemInfo}><View style={s.itemNameRow}><Text style={s.itemName}>{item.name}</Text>{!item.available && <Text style={s.soldOut}>غير متوفر</Text>}</View>{!!item.description && <Text numberOfLines={2} style={s.itemDescription}>{item.description}</Text>}<Text style={s.price}>{Number(item.price || 0).toFixed(2)} ر.س</Text></View>
    ...
    <Sheet visible={editorOpen} onClose={closeEditor} title={editingId ? 'تعديل الصنف' : 'صنف جديد'}>
      ...
      <Text style={s.label}>السعر بالريال السعودي</Text><TextInput value={draft.price} onChangeText={(price) => setDraft((old) => ({ ...old, price }))} editable={!busy} keyboardType="decimal-pad" placeholder="18" placeholderTextColor={c.muted} style={s.input} />
...
const s = StyleSheet.create({
  ...
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 30 }, content: { padding: 18, paddingBottom: 120, gap: 24 },
  empty: { alignItems: 'center', gap: 12, paddingVertical: 60 }, emptyIcon: { width: 76, height: 76, borderRadius: 27, backgroundColor: c.cream, alignItems: 'center', justifyContent: 'center' },
  ...
  footer: { position: 'absolute', left: 18, right: 18, bottom: 20 }, label: { color: c.text, fontSize: 13, fontWeight: '700', textAlign: 'right' },
