...
import { Photo, IconButton, ui } from './Kit';
...
export default function CafeCard({ cafe, onOpen }) {
  ...
  return <View style={s.card}>
    <TouchableOpacity onPress={() => onOpen(cafe)} accessibilityRole="button" accessibilityLabel={`عرض مقهى ${cafe.name}`} style={s.photo}>
      ...
      <View style={s.photoTitle}><Text style={s.latin}>{cafe.latin}</Text><Text style={s.name}>{cafe.name}</Text></View>
    </TouchableOpacity>
    <View style={s.save}><IconButton glass icon={saved.includes(cafe.id) ? 'bookmark' : 'bookmark-outline'} label={saved.includes(cafe.id) ? `إزالة ${cafe.name} من المحفوظات` : `حفظ ${cafe.name}`} selected={saved.includes(cafe.id)} onPress={() => toggleSaved(cafe.id)} /></View>
